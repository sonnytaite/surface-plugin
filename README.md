...
Open your vault in [Obsidian](https://obsidian.md) — it is just markdown, so graph
view, backlinks, search, and click-to-edit come free, and they can never disagree with
the plugin because the files *are* the state. The styled HTML/PDF digests are made to
be read beside the terminal the same way.

## Roadmap (v2 direction: lenses, not a harness)

A design constraint, learned the hard way elsewhere: a companion app that *claims to
show live system state* inevitably drifts from the files and starts lying. So surface
will grow **generated, timestamped, read-only lenses** — never a background server or a
second write path:

- a static `dashboard.html` regenerated by `/weave`: wiki counts, the
  active/drifting/dormant ladder, triage queue, disposition stats, commons health,
  links to recent digests and scan reports;
- the styled render treatment for `/scan` reports (review digests already have it);
- a config *viewer* in the dashboard (changing config stays a file edit — one write
  path);
